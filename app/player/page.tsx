...
export default function PlayerDashboard() {
  ...
  const conditionRef = useRef<Condition>(null); // ← stale closure 방지용
    ...
    const init = async () => {
      ...
      if (player) {
        ...
        if (checkin) {
          ...
          const c = map[checkin.condition_score];
          setCondition(c);
  ...
  // 컨디션: useRef로 체크해서 stale closure 완전 차단
  const handleCondition = async (selected: Condition) => {
    if (!playerId || conditionRef.current !== null) return;
  ...
  // 퀘스트 토글: 완료 → 취소 / 미완료 → 완료
  const handleQuestToggle = async (missionId: string) => {
    if (!userId) return;

    const isDone = completedMissionIds.has(missionId);

    if (isDone) {
      // 취소: user_missions에서 오늘 해당 미션 삭제
      const todayStart = `${today}T00:00:00`;
      const todayEnd = `${today}T23:59:59`;
      const { error } = await supabase
        .from('user_missions')
      ...
      <div className="px-6 mt-4">
        <div className="flex justify-between items-center mb-3">
          ...
          {!questLoading && totalCount > 0 && (
            <span className="text-xs text-gray-400 font-medium">{completedCount}/{totalCount} 완료</span>
          )}
        </div>
        ...
        {completedCount > 0 && (
          <p className="text-center text-xs text-gray-400 mt-3">완료한 퀘스트를 다시 누르면 취소할 수 있어요</p>
        )}
      </div>
